...
import {
  ...
  hapusModul, updateVideo, hapusVideo,
} from "../../api"
          ...
          {kgLoading ? (
            ...
            <div key={m.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="p-5 border-b border-gray-50 flex items-center justify-between gap-4">
                <div>
                  <h2 className="font-bold">{m.nama_domain}</h2>
                  <p className="text-[11px] text-gray-400">{m.bab.length} bab</p>
                </div>
                <button
                  onClick={() => setDeleteTarget(m)}
                  className="flex items-center gap-1.5 px-3 py-2 text-red-600 hover:bg-red-50 rounded-xl text-xs font-bold transition-colors shrink-0"
                  title="Hapus Modul"
                >
                  ...
                  <div key={bab.id}>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-[11px] font-bold bg-blue-50 text-blue-600 px-2 py-1 rounded-lg">Bab {bab.nomor}</span>
                        <span className="font-medium text-sm text-gray-800">{bab.nama}</span>
                        <span className="text-[11px] text-gray-400">{bab.jumlah_konsep} konsep</span>
                      </div>
                      <ChevronDown size={16} className={`text-gray-400 transition-transform ${expandedBab[bab.id] ? "rotate-180" : ""}`} />
                    </button>
                    {expandedBab[bab.id] && (
